...
def p_extgrammar(p):
    """extgrammar : grammar
    """
    p[0] = S('extgrammar', p.__getslice__(1, None), p.lineno(0), p.lexpos(0))
...
def p_grammar(p):
    """grammar  : def
                | def grammar
    """
    p[0] = S('grammar', p.__getslice__(1, None), p.lineno(0), p.lexpos(0))
...
def p_tokendef(p):
    ...
    """
    if len(p) > 5:
        p[0] = S('tokendef', (p[1], p[3], p[4]), p.lineno(0), p.lexpos(0))
    else:
        p[0] = S('tokendef', (p[1], p[3]), p.lineno(0), p.lexpos(0))
...
def p_tokenvalue(p):
    """tokenvalue : REGEXP
                  ...
                  | TOKEN tokenvalue
    """
    p[0] = S('tokenvalue', p.__getslice__(1, None), p.lineno(0), p.lexpos(0))

def p_tokenmods(p):
    """tokenmods : tokenmod
                 | tokenmod tokenmods
    """
    p[0] = S('tokenmods', p.__getslice__(1, None), p.lineno(0), p.lexpos(0))

def p_tokenmod(p):
    """tokenmod : LPAR OPTION modtokenlist RPAR"""
    p[0] = S('tokenmod', (p[2], p[3]), p.lineno(0), p.lexpos(0))

def p_modtokenlist(p):
    """modtokenlist : tokendef modtokenlist
                    |
    """
    p[0] = S('modtokenlist', p.__getslice__(1, None), p.lineno(0), p.lexpos(0))

def p_subgrammar(p):
    """subgrammar : LCURLY extgrammar RCURLY"""
    p[0] = S('subgrammar', [p[2]], p.lineno(0), p.lexpos(0))

def p_ruledef(p):
    """ruledef : RULENAME COLON rules_list SEMICOLON"""
    p[0] = S('ruledef', [p[1], p[3]], p.lineno(0), p.lexpos(0))
...
def p_optiondef(p):
    ...
    if len(p) == 6:
        assert p[1] == '%fragment'
        p[0] = S('fragmentdef', (p[2], p[4]), p.lineno(0), p.lexpos(0))
    else:
        p[0] = S('optiondef', (p[1], p[3]), p.lineno(0), p.lexpos(0))

def p_rules_list(p):
    """rules_list   : production
                    | production OR rules_list"""
    p[0] = S('rules_list', [p[1]] + p.__getslice__(3, None), p.lineno(0), p.lexpos(0))
...
def p_perm_rule(p):
    ...
    if len(p) == 2:
        p[0] = S('perm_rule', (p[1],), p.lineno(0), p.lexpos(0))
    else:
        p[0] = S('perm_rule', (p[1], p[3]), p.lineno(0), p.lexpos(0))

def p_perm_phrase(p):
    """perm_phrase : rule PERM rule
                   | rule PERM perm_phrase
    """
    p[0] = S('perm_phrase', [p[1]] + p.__getslice__(3, None), p.lineno(0), p.lexpos(0))
...
def p_rule(p):
    """rule : expr
            ...
            |
    """
    p[0] = S('rule', p.__getslice__(1, None), p.lineno(0), p.lexpos(0))
...
def p_oper(p):
    "oper : expr OPER"
    p[0] = S('oper', (p[1], p[2]), p.lineno(0), p.lexpos(0))
...
def parse(text, debug=False):
    lexer.lineno = 1
    return _parser.parse(text, lexer=lexer, debug=debug, tracking=True)
