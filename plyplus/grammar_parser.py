...
def p_extgrammar(p):
    """extgrammar : grammar
                  | grammar SECTION
    """
    p[0] = S('extgrammar', p[1:], p.lineno(0), p.lexpos(0))

def p_grammar(p):
    """grammar  : def
                | def grammar
    """
    p[0] = S('grammar', p[1:], p.lineno(0), p.lexpos(0))
...
def p_tokenvalue(p):
    """tokenvalue : REGEXP
                  ...
                  | TOKEN tokenvalue
    """
    p[0] = S('tokenvalue', p[1:], p.lineno(0), p.lexpos(0))

def p_tokenmods(p):
    """tokenmods : tokenmod
                 | tokenmod tokenmods
    """
    p[0] = S('tokenmods', p[1:], p.lineno(0), p.lexpos(0))
...
def p_modtokenlist(p):
    """modtokenlist : tokendef modtokenlist
                    |
    """
    p[0] = S('modtokenlist', p[1:], p.lineno(0), p.lexpos(0))
...
def p_rules_list(p):
    ...
    p[0] = S('rules_list', [p[1]] + p[3:], p.lineno(0), p.lexpos(0))
...
def p_perm_phrase(p):
    ...
    """
    p[0] = S('perm_phrase', [p[1]] + p[3:], p.lineno(0), p.lexpos(0))
...
def p_rule(p):
    """rule : expr
            ...
            |
    """
    p[0] = S('rule', p[1:], p.lineno(0), p.lexpos(0))
